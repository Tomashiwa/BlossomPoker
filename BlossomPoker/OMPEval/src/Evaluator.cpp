...
const unsigned Evaluator::RANKS[]{0x2000, 0x8001, 0x11000, 0x3a000, 0x91000, 0x176005, 0x366000,
...
const unsigned Evaluator::FLUSH_RANKS[]{1, 2, 4, 8, 16, 32, 64, 128, 256, 512, 1024, 2048, 4096};

Hand Hand::CARDS[]{};
const Hand Hand::EMPTY(0x3333ull << SUITS_SHIFT, 0);
uint16_t Evaluator::LOOKUP[]{};
uint16_t* Evaluator::ORIG_LOOKUP = nullptr;
uint16_t Evaluator::FLUSH_LOOKUP[]{};
const unsigned Evaluator::MAX_KEY = 4 * RANKS[12] + 3 * RANKS[11];
bool Evaluator::cardInit = (initCardConstants(), true);
...
void Evaluator::initCardConstants()
{
...
unsigned Evaluator::populateLookup(uint64_t ranks, unsigned ncards, unsigned handValue, unsigned endRank,
...
unsigned Evaluator::getKey(uint64_t ranks, bool flush)
{
    unsigned key = 0;
...
unsigned Evaluator::getBiggestStraight(uint64_t ranks)
{
...
void Evaluator::calculatePerfectHashOffsets()
{
...
void Evaluator::outputTableStats(const char* name, const void* p, size_t elementSize, size_t count)
{
    char dummy[64]{};
