...
c: rebuys, fixed custom amount of deals" << std::endl;
		...
		else if (c == 'c')
		{
			...
			rules.fixedNumberOfDeals = Util::strtoval<int>(s);
		}
		else if (c == 'q') return;
...
c: custom" << std::endl;
			...
			else if (c == 'c')
			{
				...
				s = getLine();
				rules.buyIn = Util::strtoval<int>(s);

				std::cout << "enter small blind: ";
				s = getLine();
				rules.smallBlind = Util::strtoval<int>(s);

				std::cout << "enter big blind: ";
				s = getLine();
				rules.bigBlind = Util::strtoval<int>(s);

				std::cout << "enter ante: ";
				s = getLine();
				rules.ante = Util::strtoval<int>(s);
		...
		else
		{
			...
			rules.bigBlind = Util::getNearestRoundNumber(rules.bigBlind);
			rules.smallBlind = rules.bigBlind / 2;

			rules.buyIn = getRandom(500, 200000);
			rules.buyIn = Util::getNearestRoundNumber(rules.buyIn);
		}
