...
#include "../inc/observer_statkeeper.h"
...
namespace OOPoker
{
	...
	*/
	...
	void applyAction(Table& table, const OOPoker::Action& action, int callAmount)
	{
		Player& player = table.players[table.current];

		if (action.command == OOPoker::A_FOLD)
		{
			player.folded = true;
		}
		else if (action.command == OOPoker::A_CHECK)
		{
			//nothing to do
		}
		else if (action.command == OOPoker::A_CALL)
		{
			placeMoney(player, callAmount);
		}
		else if (action.command == OOPoker::A_RAISE)
		{
	...
	Event eventFromAction(const OOPoker::Action& action, int callAmount, const std::string& playerName)
	{
		switch (action.command)
		{
		case OOPoker::A_FOLD: return Event(E_FOLD, playerName);
		case OOPoker::A_CHECK: return Event(E_CHECK, playerName);
		case OOPoker::A_CALL: return Event(E_CALL, playerName);
		case OOPoker::A_RAISE: return Event(E_RAISE, playerName, action.amount - callAmount);
	...
	int getInitialPlayer(Round round, int dealerIndex, const std::vector<Player>& players)
	{
		...
		*/
		...
		else if (players.size() == 2)
		{
			...
			if (round == R_PRE_FLOP) return Util::wrap(dealerIndex, players.size());
			else return Util::wrap(dealerIndex + 1, players.size());
		}
		else
		{
			...
			index = Util::wrap(index, players.size());
	...
	*/
	...
	void Game::settleBets(Table& table, Rules& rules)
	{
		...
		while (bets_running) //keeps running until all bets are settled
		{
			...
			OOPoker::Action action = player.doTurn(getInfoForPlayers(table, table.current));

			if (!isValidAction(action, player.stack, player.wager, table.getHighestWager(), table.lastRaiseAmount))
			{
				events.push_back(Event("INVALID ACTION FROM " + player.getName() + " (" + Util::valtostr(action.command) + " " + Util::valtostr(action.amount) + ")", E_DEBUG_MESSAGE));
				action = OOPoker::Action(OOPoker::A_FOLD);
			}
			...
			if (action.command == OOPoker::A_RAISE)
			{
			...
			else if (table.lastRaiser == -1 && (action.command == OOPoker::A_CALL || action.command == OOPoker::A_CHECK)) table.lastRaiser = table.current;
