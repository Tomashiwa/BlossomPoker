...
namespace OOPoker
{
	...
	Action Info::amountToAction(int amount) const
	{
		...
		if (amount < 0)
		{
			return OOPoker::Action(OOPoker::A_FOLD);
		}
		else if (amount < call)
		{
			return OOPoker::Action(OOPoker::A_FOLD);
		...
		{
			return OOPoker::Action(A_CHECK);
		}
		else if (amount == call)
		{
			return OOPoker::Action(A_CALL);
		}
		else if (amount > call && amount < raise)
		{
			return OOPoker::Action(A_CALL); //amount too small for the min raise rule
		}
		else if (amount > call)
		{
	...
	{
		return OOPoker::isValidAllInAction(action, getStack(), getWager(), getHighestWager(), minRaiseAmount);
	}

	bool Info::isValidAction(const Action& action) const
	{
		return OOPoker::isValidAction(action, getStack(), getWager(), getHighestWager(), minRaiseAmount);
	}
	...
	double Info::getPotEquity(int index) const
	{
		...
		return OOPoker::getPotEquity(getHoleCards(index), boardCards, numOpponents);
	}
